# 👋 ¡Hola! Soy Mariano Reynaga Ducos

🎓 Estudiante de la carrera **Ingeniería Informática**  
📚 Actualmente cursando **Programación II**

---

## 🖥️ Tecnologías

**Lenguajes:**  
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Go](https://img.shields.io/badge/Go-00ADD8?style=for-the-badge&logo=go&logoColor=white)

**Web:**  
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css&logoColor=white)

**Infraestructura & Tools:**  
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Tailscale](https://img.shields.io/badge/Tailscale-4433E1?style=for-the-badge&logo=tailscale&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=for-the-badge&logo=raspberry-pi&logoColor=white)
![Oracle Cloud](https://custom-icon-badges.demolab.com/badge/Oracle%20Cloud-F80000?style=for-the-badge&logo=oracle&logoColor=white)

---

## 🏠 Mi Home Lab

Actualmente gestiono mi propia infraestructura doméstica para testing y servicios personales:

* **Node Central:** Raspberry Pi corriendo servicios mediante **Docker**.
* **Networking:** Mesh VPN configurada con **Tailscale** para acceso remoto seguro.
* **DNS & Privacy:** Implementación de **Pi-hole + Unbound** para bloqueo de ads a nivel red.
* **Streaming:** Desarrollo de una plataforma privada de streaming con transcodificación dinámica.
* **Cloud Hybrid:** VPS en Oracle Cloud integrado a la red mediante **Tailscale**.

---

## 🎯 Intereses

**🎮 Gaming:** *Counter-Strike 2*, *Minecraft*  
**📺 Anime:** *Sousou no Frieren*, *Monster*  
**🎵 Listening to:** *Taylor Swift, Sabrina Carpenter, Chappell Roan, Olivia Rodrigo*

---

### ✨ My Current Era

> "Everything you lose is a step you take" 📿

¡Gracias por visitar mi perfil! Nos vemos en el próximo commit.
